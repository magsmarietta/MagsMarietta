/* =====================================================
   MAGS MARIETTA — products.js
   Single source of truth for all products.
   Edit here — shop.html and product.html both read this.
   ===================================================== */

const PRODUCTS = [
  {
    id:          "skull-tag",
    name:        "Skull Tag",
    price:       30.00,
    category:    "Zipper Pull",
    description: "Cast 316L stainless steel zipper pull.",
    details:     ["316L Stainless Steel", "Polished finish", "Fits #5 YKK & standard zippers", "Approx. 2.8cm × 1.8cm"],
    model:       "models/skull-tag.glb",
    thumbnail:   null,
    inventory:   true,  // ← shows in Inventory
    deal:        false,  // ← shows in Great Deals
    featured:    true    // ← shows in Featured Items on index
  },
  {
    id:          "mace-tag",
    name:        "Mace Tag",
    price:       30.00,
    category:    "Zipper Pull",
    description: "Mace-head zipper pull.",
    details:     ["316L Stainless Steel", "Polished finish", "Fits #5 YKK & standard zippers", "Approx. 3.2cm × 1.4cm"],
    model:       "models/mace-tag.glb",
    thumbnail:   null,
    inventory:   true,
    deal:        false,
    featured:    true
  },
  {
    id:          "spirit-tag",
    name:        "Spirit Tag",
    price:       30.00,
    category:    "Zipper Pull",
    description: "...",
    details:     ["316L Stainless Steel", "Polished finish", "Fits #5 YKK & standard zippers", "Approx. 3.0cm × 1.6cm"],
    model:       "models/spirit-tag.glb",
    thumbnail:   null,
    inventory:   true,
    deal:        false,
    featured:    true
  },
  {
    id:          "ring",
    name:        "Ring",
    price:       30.00,
    category:    "Jewelry",
    description: "Solid stainless band. No coating, no plating. The brushed finish deepens with wear.",
    details:     ["316L Stainless Steel", "Polished finish", "See chart for sizing", "..."],
    model:       "models/ring.glb",
    thumbnail:   null,
    inventory:   true,
    deal:        false,
    featured:    true
  },
  {
    id:          "aglet",
    name:        "Aglet",
    price:       30.00,
    category:    "Hardware",
    description: "Cold-formed stainless aglets. Replace the plastic tips on laces, cords, drawstrings. Built to outlast the shoe.",
    details:     ["316L Stainless Steel", "Polished finish", "Fits 3–5mm cord", "Sold as a pair"],
    model:       "models/aglet.glb",
    thumbnail:   null,
    inventory:   true,
    deal:        false,
    featured:    true
  },
  {
    id:          "jump-ring",
    name:        "Jump Ring",
    price:       3.00,
    category:    "Hardware",
    description: "Heavy-gauge stainless jump rings. For connecting, layering, customizing. Sold individually.",
    details:     ["316L Stainless Steel", "Polished finish", "8mm inner diameter", "2mm wire gauge"],
    model:       "models/jump-ring.glb",
    thumbnail:   null,
    inventory:   true,
    deal:        true,
    featured:    false
  },
  {
    id:          "desert-eagle",
    name:        "Desert Eagle",
    price:       18.00,
    category:    "Zipper Pull",
    description: "...",
    details:     ["316L Stainless Steel", "Polished finish", "Fits #5 YKK & standard zippers", "Approx. 3.2cm × 1.4cm"],
    model:       "models/desert-eagle.glb",
    thumbnail:   null,
    inventory:   false,
    deal:        true,
    featured:    true
  }
];

// Node.js compat (ignored in browser)
if (typeof module !== "undefined") module.exports = PRODUCTS;
